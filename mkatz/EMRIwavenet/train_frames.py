...
def save_waveplot(path, y_hat, y_target):
    ...
    ax1.plot(y_target, color='blue', label='target')
    ax1.plot(y_hat, color='orange', label='generator')
    ax1.set_xlabel('time (s)')
    xticks = np.arange(5)*1000
    ax1.set_xticks(xticks)
    ax1.set_xticklabels(xticks*10)

    ax1.set_ylabel('Normalized Strain')
    plt.tight_layout()
    plt.savefig(path[:-4] + '2.png', format="png")
    plt.close()
...
def eval_model(global_step, writer, device, model, y, c, g, input_lengths, eval_dir, ema=None):
    ...
    path = join(eval_dir, "frame_step{:09d}_predicted.npy".format(global_step))
    np.save(path, y_hat)
    path = join(eval_dir, "frame_step{:09d}_target.npy".format(global_step))
    np.save(path, y_target)

    # save figure
    path = join(eval_dir, "frame_step{:09d}_waveplots.png".format(global_step))
...
def get_data_loaders_back(data_root, speaker_id, test_shuffle=True):
    data_loaders = {}
...
class Dataset(data_utils.Dataset):
    def __init__(self, data_root, filename, features_name, labels_name, labels_scaler, train=False):
...
def get_data_loaders(data_root, speaker_id, batch_size=hparams.batch_size, test_shuffle=True, lims = {'ecc':[0.01, 0.7], 'inc':[0.01, 1.3962634016]}):
    ...
    for phase in ["train", "test"]:
        ...
        data_loaders[phase] = data_utils.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False)
...
if __name__ == "__main__":
    ...
    for cp in np.arange(500, 151000+500, 500):
        #checkpoint_path = args["--checkpoint"]
        if 'checkpoint_step%09d_ema.pth'%cp not in os.listdir('checkpoints/'):
            continue
        checkpoint_path = 'checkpoints/checkpoint_step%09d_ema.pth'%cp
